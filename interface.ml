...
module Interface :
	sig 
		...
		val depart : unit -> unit  

		val boucle : int -> Graphics.color list -> unit
...
let rec boucle ad tableau = 
	...
	if bu.mouse_x >= 345 && bu.mouse_x <= 425 && bu.mouse_y >= 33 && bu.mouse_y <= 73 then
		(valider bu.mouse_x bu.mouse_y ad ; pionplace (Random.int 5) (Random.int 5) ad; let tableau = (recucode 0) in if tableau != [] then rond (List.hd tableau) 10 10 ; let tableau = (recucode 0) in boucle (ad+1) tableau) else
		(cliccouleur bu.mouse_x bu.mouse_y (point_color bu.mouse_x bu.mouse_y); boucle ad tableau);;
boucle 0 [];;
